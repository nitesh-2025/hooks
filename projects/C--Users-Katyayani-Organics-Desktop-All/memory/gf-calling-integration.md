...
metadata: 
  ...
  modified: 2026-09-14T13:53:55.564Z
---
...
- Later the same day the user changed selection to routing per dialled number: domestic → Vartalap (the agent's ko-sales VoIP/CallerApp), international → Exotel, env-controlled (`VITE_CALL_ROUTE_DOMESTIC`, `VITE_CALL_ROUTE_INTERNATIONAL` = vartalap|exotel). Both providers run at once; manual provider toggle removed. Decided from the NUMBER, not lead market type.
- User rules (verbatim intent, fixed in code, not configurable): "+91 hai to domestic hi hai", "+91 nahi hai to international hi manana hai". NO cross-over fallback: Vartalap unavailable → +91 call refused with the real reason, never sent via Exotel.
- Calling's ko-sales host must be the verification portal's (prod `api.b2bsales.ko-tech.in/api/v1`, env `VITE_CALLING_KO_SALES_API_URL`), not gf's `VITE_CRM_API_URL` (beta) — the beta lookup broke Vartalap. Vercel still needs these vars.
- Disposition popup: only Call Outcome + Reason + Notes (user asked to remove Next Action / follow-up for both connected and not-connected).
